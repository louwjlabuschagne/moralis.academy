...
contract.skip("Dex: create limit orders", (accounts) => {
  //The user must have ETH deposited such that deposited eth >= buy order value
  it("should throw an error if ETH balance is too low when creating BUY limit order", async () => {
    let dex = await Dex.deployed();
    let acc1Bal = await web3.eth.getBalance(accounts[0]);
    console.log("acc1Bal: " + web3.utils.fromWei(acc1Bal));
    ...
    acc1Bal = await web3.eth.getBalance(accounts[0]);
    console.log("acc1Bal: " + web3.utils.fromWei(acc1Bal));
  ...
  it("should throw an error if token balance is too low when creating SELL limit order", async () => {
    ...
    dex.addToken(LINK_TICKER, link.address);
  ...
  it("The BUY order book should be ordered on price from highest to lowest starting at index 0", async () => {
    ...
    let orderbook = await dex.getOrderBook(LINK_TICKER, BUY_ORDER);
    assert(orderbook.length, "Order book should have orders");
  ...
  it("The SELL order book should be ordered on price from lowest to highest starting at index 0", async () => {
    ...
    let link = await Link.deployed();
    await link.approve(dex.address, 500);
    await dex.createLimitOrder(SELL_ORDER, LINK_TICKER, 1, 300);
    await dex.createLimitOrder(SELL_ORDER, LINK_TICKER, 1, 100);
    await dex.createLimitOrder(SELL_ORDER, LINK_TICKER, 1, 200);

    let orderbook = await dex.getOrderBook(LINK_TICKER, 1);
    assert(orderbook.length, "Order book should have orders");
    for (let i = 0; i < orderbook.length - 1; i++) {
      assert(
        orderbook[i].price <= orderbook[i + 1].price,
        "The SELL order book should be ordered on price lowest to higest starting at index 0"
      );
    }
  });
});

// createMarketOrder(Side side, bytes32 ticker, uint amount)
contract("Dex: Create Market Orders", (accounts) => {
